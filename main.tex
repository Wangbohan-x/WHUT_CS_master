...
\usepackage[Chinesetype=工学, Englishtype=Science, Chinesedegree=硕士, Englishdegree=Master]{WUTthesis}
...
\usepackage[colorlinks,linkcolor=black,anchorcolor=blue,citecolor=black,urlcolor=blue]{hyperref}  %设置超链接及其颜色，不想要参考文献引用有颜色的从这里修改
...
\usepackage{tabularx}
% \usepackage{algorithmicx} %%算法伪代码宏包
...
\makeatletter                 %为正文前页面设置为大写
\renewcommand{\frontmatter}{%
  \cleardoublepage
  \@mainmatterfalse
  \pagenumbering{Roman} % 大写罗马数字，替代默认的roman(小写)
}
\makeatother

\renewcommand{\contentsname}{目次} % 将目录名称改为"目次"
...
\renewcommand{\l@chapter}[2]{%
  \ifnum \c@tocdepth >\m@ne
    ...
    \begingroup
      ...
      \hskip -\leftskip
      #1\nobreak\leaders\hbox{$\mkern \@dotsep mu\zihao{-5}\hbox{.}\mkern \@dotsep mu$}\hfill % 添加引导线
...
\ctexset{chapter={format+={\zihao{-2}\heiti\raggedright}, number={\arabic{chapter}}, name={}, beforeskip={6pt},afterskip={30pt},pagestyle={fancy},fixskip=true}} %% 设置章标题为字号小二，黑体，阿拉伯数字，章节标题段前6磅，段后30pt
\CTEXsetup[format={\raggedright}]{section}%%section标题左对齐
\ctexset{section={format+={\zihao{3}\heiti},beforeskip={6pt}, afterskip={18pt}}} %% 设置节标题为字号三号，黑体，阿拉伯数字，章节标题段前6磅，段后18磅
\CTEXsetup[format={\raggedright}]{subsection}
\ctexset{subsection={format+={\zihao{4}\heiti},beforeskip={3pt},afterskip={12pt}}} %% 设置二级节标题为字号四号，黑体，阿拉伯数字，章节标题段前3pt，段后12pt
...
\let\oldtableofcontents\tableofcontents    % 目录标题格式定制：居中、黑体小三（临时切换，不影响正文）
\renewcommand{\tableofcontents}{%
  \CTEXsetup[format={\centering\zihao{-3}\heiti}]{chapter}%
  \oldtableofcontents
  \CTEXsetup[format={\zihao{-2}\heiti\raggedright}]{chapter}%
}

\addbibresource{biber.bib} %% 加入参考文献书库库文件

% \setCJKmainfont{SimSun}                % 中文正文：宋体
% \setmainfont{Times New Roman}          % 西文正文：Times
